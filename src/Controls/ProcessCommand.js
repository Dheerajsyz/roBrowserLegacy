...
define(function (require) {
	...
	let CommandStore = {
		...
		navi: {
			...
			callback: function (text) {
				var matches = text.match(/^navi\s+(\S+)\s+(\d+)\s+(\d+)/);
				if (matches) {
					var Navigation = getModule("UI/Components/Navigation/Navigation");
		...
		alchemist: {
			...
			callback: function () {
				var pkt = new PACKET.CZ.RANKING();
				pkt.rankType = 1; // Alchemist
				Network.sendPacket(pkt);
				return;
			},
		},
