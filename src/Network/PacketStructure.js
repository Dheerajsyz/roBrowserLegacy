...
define(['Utils/BinaryWriter', './PacketVerManager', 'Utils/Struct', 'Core/Configs'], function (BinaryWriter, PACKETVER, Struct, Configs) {
	...
	// 0x1cd / 0x0AFB (PACKETVER >= 20181128)
	// ZC_AUTOSPELLLIST - variable length
	// rAthena: <packetType>.W <packetLen>.W <skills>.L * N
	// Note: NetworkManager already handles packet length, use end parameter
	PACKET.ZC.AUTOSPELLLIST = function PACKET_ZC_AUTOSPELLLIST(fp, end) {
		var count = (end - fp.tell()) >> 2; // remaining bytes / 4

		var out = new Array(count);
		for (var i = 0; i < count; ++i) {
			out[i] = fp.readLong(); // int32 skill id
		}
		this.SKID = out;
	};
	PACKET.ZC.AUTOSPELLLIST.size = -1;
	...
	*/
	...
	// 0xa46 CZ_REQ_STYLE_CHANGE - Client requests a style change
	PACKET.CZ.REQ_STYLE_CHANGE = function PACKET_CZ_REQ_STYLE_CHANGE() {
		this.HeadPalette = 0;
		this.HeadStyle   = 0;
		this.BodyPalette = 0;
		this.TopAcc      = 0;
	...
	PACKET.CZ.REQ_STYLE_CHANGE.prototype.build = function() {
		var pkt_buf = new BinaryWriter(14);
		pkt_buf.writeShort(0x0a46);
		pkt_buf.writeShort(this.HeadPalette);
		pkt_buf.writeShort(this.HeadStyle);
		pkt_buf.writeShort(this.BodyPalette);
		pkt_buf.writeShort(this.TopAcc);
		pkt_buf.writeShort(this.MidAcc);
		pkt_buf.writeShort(this.BotAcc);
		return pkt_buf;
	};

	// 0xa47 ZC_STYLE_CHANGE_RES - Server response to style change request
	PACKET.ZC.STYLE_CHANGE_RES = function PACKET_ZC_STYLE_CHANGE_RES(fp, end) {
		this.flag = fp.readByte();
	};
	PACKET.ZC.STYLE_CHANGE_RES.size = 3;

	// 0xa48 ZC_CLOSE_UI_STYLINGSHOP - Server asks client to close the stylist window
	PACKET.ZC.CLOSE_UI_STYLINGSHOP = function PACKET_ZC_CLOSE_UI_STYLINGSHOP(fp, end) {};
	PACKET.ZC.CLOSE_UI_STYLINGSHOP.size = 2;
	...
	PACKET.CZ.UPLOAD_MACRO_DETECTOR_CAPTCHA.prototype.build = function() {
		var imgLen = this.imageData ? this.imageData.byteLength : 0;
		var pkt_len = 2 + 2 + 4 + imgLen;
		var pkt_buf = new BinaryWriter(pkt_len);
	...
	// 0x0a55
	PACKET.ZC.COMPLETE_UPLOAD_MACRO_DETECTOR_CAPTCHA = function PACKET_ZC_COMPLETE_UPLOAD_MACRO_DETECTOR_CAPTCHA(fp, end) {};
