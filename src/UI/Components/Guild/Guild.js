...
Guild.init = function init() {
	...
	ui.find('.content.info .emblem_edit input').change(function () {
		...
		// Browsers report BMP with various MIME types; also accept by extension as fallback
		const validBmpTypes = ['image/bmp', 'image/x-bmp', 'image/x-ms-bmp', 'image/x-windows-bmp'];
		const isBmp = file && (validBmpTypes.indexOf(file.type) !== -1 || (file.name && file.name.toLowerCase().endsWith('.bmp')));
		const isGif = file && (file.type === 'image/gif' || (file.name && file.name.toLowerCase().endsWith('.gif')));
		if (this.files.length && (isBmp || isGif) && file.size <= 50000) {
			...
			console.warn('[Warning] Incorrect emblem file type or size. Only BMP/GIF files (<=50KB) are accepted.');
			ChatBox.addText('Emblem upload failed: please select a BMP or GIF file (max 50KB).', ChatBox.TYPE.ERROR, ChatBox.FILTER.PUBLIC_LOG);
		}
	});
