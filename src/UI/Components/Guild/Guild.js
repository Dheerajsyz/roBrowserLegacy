...
define(function(require)
{
	...
	Guild.init = function init()
	{
		...
		ui.find('.content.info .emblem_edit input').change(function(){
			...
			var validBmpTypes = ['image/bmp', 'image/x-bmp', 'image/x-ms-bmp', 'image/x-windows-bmp'];
			var isBmp = file && (validBmpTypes.indexOf(file.type) !== -1 || (file.name && file.name.toLowerCase().endsWith('.bmp')));
			var isGif = file && (file.type === 'image/gif' || (file.name && file.name.toLowerCase().endsWith('.gif')));
